...
def main():
    ...
    missed_episodes = 0
    current_episode = 0
    out_path="/root/dataset/curobo_panda_pick_and_place_ghost_robot_dataset.parquet"
    ...
    #  Robot at [0,0,0]
    #  collision robot will be at [1,0,0]
    #                   x
    ...
    # add wall behind robot (avoid trajectories that go this way)
    wall = cuboid.DynamicCuboid(
        "/World/wall",
        position=np.array([-0.5, 0.0, 1.0]),
        orientation=np.array([0, 1, 0, 0]),
        color=np.array([1.0, 1.0, 1.0]),
        scale=np.array([0.01, 2, 2]),
    )
    ...
    obstacle_pick_place.disable_rigid_body_physics()
    obstacle_pick_place.set_collision_enabled(True)
    ...
    obstacle_df = {}
    obstacle_df["sphere_0_x"] = obstacle_position_x
    obstacle_df["sphere_0_y"] = obstacle_position_y
    obstacle_df["sphere_0_z"] = obstacle_position_z
    obstacle_df["sphere_0_radius"] = obstacle_radius
    ...
    target_retract_pos = retract_state.ee_position[0]
    target_retract_rot = retract_state.ee_quaternion[0]
    ...
    if simulation_app.is_running():
        ...
        while current_episode < nr_episodes:
            ...
            obstacle.set_world_pose(
                position=np.array([obstacle_position_x, obstacle_position_y, obstacle_position_z]).reshape(3),
                orientation=np.array([0, 1, 0, 0]),
            )
            ...
            while not converged_retract:
                ...
                if cmd_plan is not None:
                    ...
                    current_df["sphere_0_x"] = obstacle_position_x
                    current_df["sphere_0_y"] = obstacle_position_y
                    current_df["sphere_0_z"] = obstacle_position_z
                    current_df["sphere_0_radius"] = obstacle_radius
                    ...
                    if (not converged_pick) and (cmd_idx >= len(cmd_plan.position)):
                        ...
                        if succ:
                            ...
                            converged_retract = True # skip this episode
                        ...
                        # set target position for place phase
                        target.set_default_state(
                            position=target_retract_pos.cpu().numpy(),
                            orientation=target_retract_rot.cpu().numpy(),
                        )
                        target.post_reset()

                        # compute curobo solution:
                        ik_goal = Pose(
                            position=tensor_args.to_device(target_retract_pos),
                            quaternion=tensor_args.to_device(target_retract_rot),
                        )
