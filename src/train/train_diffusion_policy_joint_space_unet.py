...
class TrainConfig:
    ...
    wandb_group: str = 'Diffusion_Policy_Joint_Space_Unet'
...
def train():
    ...
    if config.weighted_sampling:
        ...
        print(f"weights: {weights[60:80]}, sum: {np.sum(weights)}")
            ...
            if (epoch_idx+1) % config.eval_step_interval == 0 and config.run_eval:
                ...
                with torch.no_grad():
                    ...
                    for eval_i, eval_batch in tqdm(enumerate(eval_dataloader), desc="Eval"):
                        ...
                        obs_cond = nobs
                        # (B, obs_horizon * obs_dim + target_dim)
                        obs_cond = obs_cond.flatten(start_dim=1)
                        # obs_cond = torch.cat([ntarget, obs_cond], dim=1)
