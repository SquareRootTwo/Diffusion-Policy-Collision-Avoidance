...
class EvalConfig:
    ...
    # run_timestamp: str = "2024-04-26_02-07-12" # transformer ik
    
    model_type: str = "unet"
    run_timestamp: str = "2024-04-18_19-46-27" # unet ik
...
def main():
    ...
    logging_out_base_path = os.path.join(root_path, f"src/data/thesis_eval/{config.run_timestamp}_{config.model_type}_cartesian_space")
            ...
            for step in range(34):
                ...
                if step % config.action_horizon == 0:
                    ...
                    with torch.no_grad():
                        ...
                        naction = torch.cat([torch.zeros_like(naction, device=device), naction], dim=2)
                        action_pred = unnormalize_data(naction, stats=action_stats).squeeze(0)

                    ee_pos_pred = action_pred[:, 7:10].cpu()
                    ee_rot_pred = action_pred[:, 10:14].cpu()
                    # convert ee_rot_pred quaternion to scipy format w, i, j, k -> i, j, k, w
                    ee_rot_pred_scipy = torch.cat([ee_rot_pred[:, 1:], ee_rot_pred[:, 0:1]], dim=1)
                ...
                step_rotation_scipy = ee_rot_pred_scipy[step, :].cpu().numpy()

                # rotate rotation around 45 degree (pybullet ik solver needs this as correct rotation)
                q = R.from_quat(step_rotation_scipy)
                q = q * R.from_euler('z', 45, degrees=True)
                step_rotation_scipy = q.as_quat()

                # set panda pose 
                ik_joint_solution = p.calculateInverseKinematics(
                    robot_id,
                    7,
                    ee_pos_pred[step, :].cpu().numpy(),
                    step_rotation_scipy,
                )
                ...
                p.setJointMotorControlArray(
                    ...
                    targetPositions=ik_joint_solution[:7]
                )
                ...
                if config.collect_data:
                    ...
                    for i in range(7):
                        ...
                        current_df[f"pred_panda_joint{i+1}"] = ik_joint_solution[i]
