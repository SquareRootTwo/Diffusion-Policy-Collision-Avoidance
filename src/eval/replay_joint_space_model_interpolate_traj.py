...
def main():
    ...
    logging_out_base_path = os.path.join(root_path, f"src/data/thesis_eval/{config.run_timestamp}_{config.model_type}_trajectory_interpolation")
        ...
        for phase in range(1, 4):
            ...
            for step in range(34):
                ...
                if step % config.action_horizon == 0:
                    if pred_action_joints is not None:
                        prev_action_joints = pred_action_joints.clone()
                    ...
                    # interpolate steps from previous and current joint actions
                    if pred_action_joints is not None:
                        pred_action_joints = action_pred[:, :7].cpu()
                        pred_action_joints[step, :] = 0.33 * pred_action_joints[step, :] + (1.0 - 0.33) * prev_action_joints[step, :]

                        if step < 33:
                            pred_action_joints[step+1, :] = (1.0 - 0.33) * pred_action_joints[step+1, :] + 0.33 * prev_action_joints[step+1, :]
                    else:
                        pred_action_joints = action_pred[:, :7].cpu()


                    ee_pos_pred, ee_rot_pred = pk.get_ee_pose(pred_action_joints.to(pk.device))
                ...
                pre_action_step = pred_action_joints[step, :]
